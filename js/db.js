...
export function isConfigured() { return _client !== null; }

function token() { return (window.APP_CONFIG || {}).appToken || ''; }

export function setCurrentGame(id) {
    id ? sessionStorage.setItem('currentGameId', id) : sessionStorage.removeItem('currentGameId');
}
...
export async function saveGame({ name, date, opponent, venue, rink, homeName, awayName }) {
...
export async function deleteGameEvents(gameId) {
    if (!_client) return;
    try {
        await _client.from('events').delete().eq('game_id', gameId);
    } catch (e) { console.warn('db.deleteGameEvents:', e); }
}
